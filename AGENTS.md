...
- **Parallel fetching & reranking:** The `all` tab uses `MergedSearchService` to fetch Primo and TIMDEX concurrently via `Thread.new`, then passes both result sets to the `reranker` gem for scoring and ordering. The scorer is configurable via `ALL_TAB_SCORER` (default: `zscore`); the number of results fetched per source is controlled by `ALL_TAB_RESULTS_PER_SOURCE` (default: 50). There is no pagination on the all tab — results are returned as a flat reranked list. Be careful when refactoring to preserve thread-safety and error handling.
